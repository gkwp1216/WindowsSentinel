...
namespace WindowsSentinel
{
    ...
    public partial class Page1 : Page
    {
        ...
        // 툴팁이 항상 컨트롤 중앙 아래에, 화면 밖으로 나가지 않게 표시되도록 커스텀 위치 지정 (개선)
        private System.Windows.Controls.Primitives.CustomPopupPlacement[] OnCustomToolTipPlacement(System.Windows.Size popupSize, System.Windows.Size targetSize, System.Windows.Point offset)
        {
            // 아이콘 중앙 아래에 위치
            double x = (targetSize.Width - popupSize.Width) / 2;
            double y = targetSize.Height + 5;
            ...
            // 현재 윈도우의 실제 위치를 고려
            var border = Keyboard.FocusedElement as FrameworkElement;
            if (border != null)
            {
                var pointOnScreen = border.PointToScreen(new System.Windows.Point(x, y));
                // 오른쪽 경계 체크
                if (pointOnScreen.X + popupSize.Width > screenWidth)
                    x -= (pointOnScreen.X + popupSize.Width) - screenWidth + 10;
                // 아래쪽 경계 체크
                if (pointOnScreen.Y + popupSize.Height > screenHeight)
                    y -= (pointOnScreen.Y + popupSize.Height) - screenHeight + 10;
            }
